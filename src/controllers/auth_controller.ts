...
const refresh = async (req: AuthRequest, res: Response) => {
  ...
  jwt.verify(
    ...
    async (err, user: { _id: string }) => {
      if (err) {
        console.log(err);
        return res.sendStatus(401); // אחרי שליחת התגובה, פונקציה תפסיק עם return
      }
      ...
      try {
        ...
        if (!userDb.tokens || !userDb.tokens.includes(refreshToken)) {
          userDb.tokens = [];
          await userDb.save();
          return res.sendStatus(401); // גם כאן, הוספת return לאחר שליחת תגובה
        }
        ...
        console.error(err);
        return res.sendStatus(500).send(err); // הוספת return למניעת ניסיון של שליחת מספר תגובות
      }
    }
  );
};
